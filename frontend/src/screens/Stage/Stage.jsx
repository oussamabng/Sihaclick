...
const Stage = (props) => {
  ...
  const [typeStageEtudiant,setTypeStaegEtudiant] = useState("Stage de fin d’étude");
  ...
  useEffect(()=>{
    ...
    .catch(err=>{
      ...
      instance2.get("public/speciality")
    .then(res=>{
      let tempArrWilaya = []
      res.data.map(elm=>{
...
setTwelve(true)
  ...
  const postStage = ()=>{
    ...
    let internship_field = data[intern_type].types.filter(elm=>elm.name.toLowerCase().includes(typeActor.toLowerCase()))[0].id
    ...
    let college_internship = {
      ...
      year:type==="Diplômé"?year :typeStageEtudiant === "Stage de fin d’étude"?parseInt( (new Date().getFullYear() + 7) - parseInt(year) +1) :null
    }
    let flexible = flexable;
    let date = flexableDate;
    let period = one?1:two?2:three?3:four?4:five?5:six?6:seven?7:eight?8:nine?9:ten?10:eleven?11:twelve?12:1
    let spec = specialitiz.filter(elm=>elm.value.toLowerCase().includes(speciality.toLowerCase()))[0].key
    console.log({intern_type,internship_field,college_internship,months,period,internship_type,flexible,date,communeId,speciality,typeActor,type,spec})
    ...
    let urlType = type==="Etudiant"?"student":type==="Diplômé"?"degre":"residant"
    instance.post(`chaab/donnation/internship/${urlType}`,body)
    .then(res=>{
      console.log(res)
    })
    .catch(err=>{
